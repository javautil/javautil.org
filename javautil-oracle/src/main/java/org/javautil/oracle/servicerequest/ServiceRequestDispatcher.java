...

{
	...
	synchronized public void exceptionHandler(final String message,
	...
	public void run() {
		...
		if (dbc == null) {
			...
			throw new IllegalStateException(
					"dataSource returned null connection");
			...
			try {
				...
				if (provider != null) {
					try {
						...
						final Class c = Class.forName(provider.getClassname());// get
																				// class
																				// def
						final ServiceRequest obj = (ServiceRequest) c
								.newInstance();
					...
					throw new java.lang.IllegalStateException("service "
							+ request.getServiceName() + " is not resolvable");
				}
			} catch (final java.sql.SQLException s) {
				boolean databaseUnavailable = dbc
						.isDatabaseUnavailableException(s);
				while (databaseUnavailable) {
					...
					try {
						...
						databaseUnavailable = dbc
								.isDatabaseUnavailableException(e);
						// todo create DataSourceWrapper interface with
						// getName()
						logger.warn("database not available " + e.getMessage()
								+ " will retry in " + retryConnectionSeconds
								+ " seconds");
				...
				exceptionHandler("ServiceRequestDispatcher " + e.getMessage(),
						e);
			}
		}
	}
