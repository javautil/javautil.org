...
class ServiceRequestPipeListener {
	private static final Logger logger = Logger
			.getLogger(ServiceRequestPipeListener.class.getName());
	 ...
	 */
	public ServiceRequestBean get(final Connection connection)
			throws java.sql.SQLException {
		if (connection == null) {
			throw new IllegalArgumentException("connection is null");
		}
		final OracleConnectionUtil wrappedConn = new OracleConnectionUtil(
				connection);
		...
		try {
			...
			while (waiting && ok) {
				...
				case 1: // timeOut, no request within 60, seconds
					logger.debug("no request received in 60 seconds on pipe '"
							+ requestPipeName + "'");
					break;
				// throw new
				...
				default:
					throw new IllegalStateException(
							"unknown return value from read pipe " + callRc);
				}
			}
