...
public class ServiceRqstBaseS {
	...
	static void getRow(final ResultSet rset, final ServiceRqstBean row)
		...
		try {
			final String ServiceRqstCd = rset
					.getString(columnName = "service_rqst_cd");
			row.setServiceRqstCd(rset.wasNull() ? null : ServiceRqstCd);

			final String ServiceRqstDescr = rset
					.getString(columnName = "service_rqst_descr");
	...
	// /**
	// * ConnectionPool safe method for persisting an instance of
	// ServiceRqstBase
	// * if this is called repeatedly within a transaction, it is highly
	// recommended
	// * that the method pairs connectionPersistenceBegin and
	// connectionPersistenceEnd be called
