...
public class OracleObjectDAO {
	 ...
	 * @throws SQLException
	 */
	public List<OracleObject> getForLikeOwner(final Connection conn,
			final DdScope scope, final String owner) throws SQLException {
		final ArrayList<OracleObject> objects = new ArrayList<OracleObject>();

		final String selectText = "select " + columnList + " from "
				+ scope.toString() + "_objects where owner like upper(:owner)";
	...
	public static void getRow(final ResultSet rset, final OracleObject row)
