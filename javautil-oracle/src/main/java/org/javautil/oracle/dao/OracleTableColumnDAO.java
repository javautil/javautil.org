...
public class OracleTableColumnDAO {
	...
	static final String columnList = "" + "    owner,\n" + "    table_name,\n"
			+ "    column_name,\n" + "    data_type,\n"
			+ "    data_type_mod,\n" + "    data_type_owner,\n"
			+ "    data_length,\n" + "    data_default,\n"
			+ "    data_precision,\n" + "    data_scale,\n" + "    nullable,\n"
	...
	// public static List<OracleTableColumn> getForTableNoUpper(Connection
	// conn,String owner, String tableName) {
	// return getForTable(conn,owner, tableName, false);
	// }

	public static List<OracleTableColumn> getForTable(Connection conn,
			String owner, String tableName, boolean useDba,
			boolean upperCaseArgs) throws SQLException {
		if (conn == null) {
		...
		String myTableName = upperCaseArgs ? tableName.toUpperCase()
				: tableName;
		...
		try {
			...
			throw new SQLException("while processing '" + selectText + "' "
					+ sqe.getMessage());
		}
		while (rset.next()) {
	...
	public static void getRow(ResultSet rset, OracleTableColumn row)
		...
		try {
			...
			row.setDataType(rset.getInt(columnName = "DATA_TYPE"));
			...
			row.setCharacterSetName(rset
					.getString(columnName = "CHARACTER_SET_NAME"));

			row.setCharColDeclLength(rset
					.getInt(columnName = "CHAR_COL_DECL_LENGTH"));
