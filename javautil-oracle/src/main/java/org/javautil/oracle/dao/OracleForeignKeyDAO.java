...
import org.javautil.jdbc.metadata.JDBCForeignKeys;
import org.javautil.jdbc.metadata.containers.ForeignKeys;
import org.javautil.oracle.metadata.OracleConstraint;
import org.javautil.oracle.metadata.dao.OracleConstraintsDAO;

public class OracleForeignKeyDAO extends JDBCForeignKeys implements ForeignKeys {

	// todo finish this off
	// select
	String columns = "select " + " ac.OWNER, " + " ac.CONSTRAINT_NAME, "
			+ //
			" ac.CONSTRAINT_TYPE, "
			+ //
			" ac.TABLE_NAME, "
			...
			+ //
			"       ac.table_name like upper(:table_name) and "
			+ "       ac.owner like upper(:owner)";
	...
	// 14. DEFERRABILITY short => can the evaluation of foreign key constraints
	// be deferred until commit
	// * importedKeyInitiallyDeferred - see SQL92 for definition
	// * importedKeyInitiallyImmediate - see SQL92 for definition
	// * importedKeyNotDeferrable - see SQL92 for definition

	public OracleForeignKeyDAO(Connection conn, String schemaName,
		...
		PreparedStatement ps = conn.prepareStatement(columns);
		ps.setString(1, tableName);
		ps.setString(2, schemaName);
		ResultSet rset = ps.executeQuery();
		while (rset.next()) {
			OracleConstraintsDAO.getImportedKeyConstraints(constraints,
