 ...
 * 
 */
public class OracleDataSourceWrapperArguments
// implements OracleDataSourceWrapperValuesSourceInterface
{
	...
	/**
	 * The name of the server or the server IP address.
	 * 
	 * The host name of the server running the Oracle Database.
	 */
	@Required
	@Exclusive(property = "url")
	private String serverName;

	/**
	 * The sql listener IP port number.
	 */
	@Optional
	@Exclusive(property = "url")
	private Integer portNumber = 1521;

	/**
	 * The oracle username for the connections.
	 */
	@Required
	private String user;
	 ...
	 */
	@Optional
	private Integer abandonedConnectionTimeout = 0;
	 ...
	 */
	@Optional
	private Integer initialLimit = 0;
	 ...
	 */
	@Optional
	private Integer maxLimit = Integer.MAX_VALUE;
	...
	/**
	 * 
	 * Sets the maximum time in seconds that a physical connection can remain
	 * idle in a connection cache. An idle connection is one that is not active
	 * and does not have a logical handle associated with it. When
	 * InactivityTimeout expires, the underlying physical connection is closed.
	 * However, the size of the cache is not allowed to shrink below minLimit,
	 * if has been set.
	 * 
	 */
	@Optional
	...
	/**
	 * Validates that the connection is valid.
	 * 
	 * Todo check to see what happens when this set.
	 */
	@Optional
	...
	/**
	 * The callback method to be invoked when a connection is abandoned or
	 * returned.
	 * 
	 * The class must implement OracleConnectionCacheCallback
	 */
	 ...
	 */
	@Optional
	private boolean useThinConnection = true;

	@Optional
	private Integer connectionWaitTimeout;

	/**
	 * The jdbc Connection required logWriter
	 * 
	 */
	...
	public OracleDataSourceWrapperArguments() {

	}
	// TODO NUKE
	// public OracleDataSourceWrapperArguments (final
	// OracleDataSourceWrapperValuesSourceInterface vals) {
	// final OracleDataSourceWrapperValues retval = new
	// OracleDataSourceWrapperValues();
	 ...
	 * 
	 * @see
	 * com.dbexperts.oracle.datasource.OracleDataSourceWrapperValuesSourceInterface
	 * #
	 * setAbandonedConnectionCallback(oracle.jdbc.pool.OracleConnectionCacheCallback
	 * )
	 */
	// public void setAbandonedConnectionCallback(final
	// OracleConnectionCacheCallback callback) {
	 ...
	 */
	public void setAbandonedConnectionTimeout(
			final int abandonedConnectionTimeout) {
	 ...
	 * 
	 *            Sets how many connections are created in the cache when it is
	 *            created or reinitialized.
	 * 
	 *            When this property is set to an integer value greater than 0,
	 *            creating or reinitializing the cache automatically creates the
	 *            specified number of connections, filling the cache in advance
	 *            of need.
	 * 
	 *            Default: 0
	 */
	public void setInitialLimit(final int initialLimit) {
		if (initialLimit < 0) {
			throw new IllegalArgumentException(
					"initialLimit must be >= 0 not: " + initialLimit);
		}
	 ...
	 * 
	 *            The default value is Integer.MAX_VALUE, meaning that there is
	 *            no limit enforced by the connection cache, so that the number
	 *            of connections is limited only by the number of database
	 *            sessions configured for the database.
	 */
	public void setMaxLimit(final int maxLimit) {
		if (maxLimit <= 1) {
			throw new IllegalArgumentException("maxLimit must be >= 1 not: "
					+ maxLimit);
		}
	 ...
	 * @param minLimit
	 *            the minLimit to set
	 * @see org.javautil.oracle.datasources.dbexperts.oracle.datasource.OracleDataSourceWrapper#setMinLimit(Integer)
	 */
	public void setMinLimit(final int minLimit) {
		if (minLimit < 0) {
			throw new IllegalArgumentException("minLimit must be >= 0 not: "
					+ minLimit);
		}
	...
	public void setInactivityTimeout(final int inactivityTimeout) {
		if (inactivityTimeout < 0) {
			throw new IllegalArgumentException(
					"inactivityTimeout must be >= 0 not: " + inactivityTimeout);
		}
	 ...
	 * @throws SQLException
	 */
	public void setConnectionCachingEnabled(
			final boolean connectionCachingEnabled) {
	...
	// /* (non-Javadoc)
	// * @see
	// com.dbexperts.oracle.datasource.OracleDataSourceWrapperValuesSourceInterface#getConnectionType()
	// */
	...
	public void setLowerThresholdLimit(final int lowerLimit) {
		if (lowerLimit < 0) {
			throw new IllegalArgumentException(
					"lowerThresholdLimit must be >0 not: " + lowerLimit);
		}
	...
	// //final OracleConnectionCacheCallback callback) {
	...
	public String getValuesText() {
		...
		b.append(ODSWrapperInterface.ServerName + ": " + getServerName()
				+ newline);
		b.append(ODSWrapperInterface.PortNumber + ": " + getPortNumber()
				+ newline);
		b.append(ODSWrapperInterface.User + ": " + getUser() + newline);
		b.append(ODSWrapperInterface.ConnectionCachingEnabled + ": "
				+ getConnectionCachingEnabled() + newline);
		b.append(ODSWrapperInterface.ServiceName + ": " + getServiceName()
				+ newline);
		b.append(ODSWrapperInterface.LowerThresholdLimit + ": "
				+ getLowerThresholdLimit() + newline);
		b.append(ODSWrapperInterface.LoginTimeout + ": " + getLoginTimeout()
				+ newline);
		b.append(ODSWrapperInterface.AbandonedConnectionTimeout + ": "
				+ getAbandonedConnectionTimeout() + newline);
		b.append(ODSWrapperInterface.InitialLimit + ": " + getInitialLimit()
				+ newline);
		b.append(ODSWrapperInterface.MinLimit + ": " + getMinLimit() + newline);
		b.append(ODSWrapperInterface.MaxLimit + ": " + getMaxLimit() + newline);
		b.append(ODSWrapperInterface.InactivityTimeout + ": "
				+ getInactivityTimeout() + newline);
		b.append(ODSWrapperInterface.InitialLimit + ": " + getInitialLimit()
				+ newline);
		b.append(ODSWrapperInterface.MaxStatementsLimit + ": "
				+ getMaxStatementsLimit() + newline);
		b.append(ODSWrapperInterface.PropertyCheckInterval + ": "
				+ getPropertyCheckInterval() + newline);
		b.append(ODSWrapperInterface.ValidateConnection + ": "
				+ isValidateConnection() + newline);
		// b.append(ODSWrapperInterface.AbandonedConnectionTimeout + ": " +
		// getAbandonedConnectionTimeout() + newline);
		return b.toString();
	}
